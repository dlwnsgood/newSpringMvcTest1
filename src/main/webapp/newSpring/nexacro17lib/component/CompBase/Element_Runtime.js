			...
			if (handle) {
				...
				if (!ctrlkey && !altkey && charcode) {
					...
					if (inputChar.length > 0) {
						if (pThis.autoskip && _range[0] == _range[1]) {
							if (pThis.maxlength === 0 && charcode == nexacro.Event.KEY_ENTER) {
		...
		__pWebBrowserPluginElement._setUrl = function (url, useurlhistory) {
			if (this._plugin_object) {
				...
				if (useurlhistory == false) {
					var _url;
					if (nexacro._OS == "Android" && nexacro._Browser == "Runtime") {
						_url = "javascript:if(location.href==\"about:blank\"){location.replace(\"" + url + "\");}else{history.replaceState(null, \"" + url + "\", \"" + url + "\");history.go(0);}";
					}
					else {
						if (url == "about:blank") {
